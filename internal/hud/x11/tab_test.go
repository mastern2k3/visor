...
// TestStateElapsed_TruncatesToWholeSeconds pins the truncation that keeps
// render()'s equality-based skip-check meaningful: two calls within the same
// second must produce an identical duration, or every snapshot broadcast
// (even one carrying no observable change for this tab) would defeat the
...
// one, so tickElapsed must re-render.
